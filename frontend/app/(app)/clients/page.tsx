  ...
  return (
    <main className="grid gap-5 xl:grid-cols-[360px_1fr]">
      <form onSubmit={handleSubmit} className="rounded-[28px] border border-white/10 bg-white/5 p-5">
        ...
        <input className="mt-4 w-full rounded-2xl border border-white/15 bg-black/25 px-4 py-3 text-sm" placeholder="Nome do cliente" value={form.name} onChange={(event) => setForm((previous) => ({ ...previous, name: event.target.value }))} />
        <input className="mt-3 w-full rounded-2xl border border-white/15 bg-black/25 px-4 py-3 text-sm" placeholder="Segmento" value={form.segment} onChange={(event) => setForm((previous) => ({ ...previous, segment: event.target.value }))} />
        <input className="mt-3 w-full rounded-2xl border border-white/15 bg-black/25 px-4 py-3 text-sm" placeholder="https://site-do-cliente.com.br" value={form.website_url} onChange={(event) => setForm((previous) => ({ ...previous, website_url: event.target.value }))} />
