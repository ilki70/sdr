...
def test_process_whatsapp_inbound_uses_message_id_as_external_message_id(monkeypatch) -> None:
    ...
    conversation = SimpleNamespace(id="conv-1", agent_id="agent-1", status=None, pipeline_status=None)
    ...
    async def fake_refresh_context(*_args, **_kwargs):
        return ConversationContextSnapshot(tenant_id="tenant-1", conversation_id="conv-1", summary="ctx")
    ...
    monkeypatch.setattr(whatsapp_gateway, "run_configured_sales_runtime", fake_run_configured_sales_runtime)
...
def test_process_whatsapp_inbound_uses_langgraph_runtime_when_enabled(monkeypatch) -> None:
    integration = SimpleNamespace(id="integration-1", webhook_secret_enc=b"secret", agent_id="agent-1")
    lead = SimpleNamespace(id="lead-1", phone=None, name=None, cpf=None, metadata_json={})
    conversation = SimpleNamespace(id="conv-1", agent_id="agent-1", pipeline_status=None, status=None)
    saved_messages: list[dict] = []
    ...
    async def fake_refresh_context(*_args, **_kwargs):
        return ConversationContextSnapshot(tenant_id="tenant-1", conversation_id="conv-1")
    ...
    assert response.reply_text == "Você está buscando imóvel ou veículo?"
    assert saved_messages[-1]["sender_type"] == "assistant"
